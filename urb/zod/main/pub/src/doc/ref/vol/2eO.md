...
Accpet a nock subject-formula cell. Produce a unit result, treating 11
as a crash (i.e. pure nock).
...
Accepts a nock subject-formula cell and an %iron gate which accepts any
noun and produces a unit (this is used as nock 11). Produces a ++toon,
which is a sucesful, blocked, or crashed result.
...
Work just like in `++makc`, but accept a `++sky`. Produce a unit
computation result.
