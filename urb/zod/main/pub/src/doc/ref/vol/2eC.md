...
Build gate to parse a nail with a rule, then replaced the parsed texted
with a constant.
...
Build gate to parse a nail with a rule, then slam a gate with the parsed
text.
...
Succeed but consume no characters - Produce an edge at the same text
position with the text to parse unchanged, but with a
...
Fail to parse - Produce a nail at the same text position but with null
text.
...
Demand politely that the parsing rule parse the entire sample nail,
produce a null edge otherwise.
...
Conditional cook - Produce the slam of the parsed texted to `b` only if
the result is not null. Else, produce null.
