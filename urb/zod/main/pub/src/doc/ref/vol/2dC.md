...
Container arm for queue operation arms. The contained arms inherit it's
sample queue, 'a'.
...
Accept a list `b` of elements of the type of the queue `a` elements and
produce the queue `a` with the elements of `b` added.
...
Accept any noun and adds to the queue as the head, producing the
resutling queue.
