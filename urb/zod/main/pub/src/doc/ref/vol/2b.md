...
Fmap; Accept a gate that accepts and produces an unwrapped value and
pass it a unit value, then produce a unit value.
...
Applies the loobean gate `b` to every element of the list `a`, producing
the logical AND of all the results.
...
Accepts a list of units and a gate. Produces the list with the gate
applied to each unit of the list that has a value; nulls are discarded.
...
Right fold - Move right to left recursively slamming a binary gate with
an element from the list and an accumulator, producing the final value
of the accumulator.
...
Left fold - Move left to right recursively slamming a binary gate with
an element from the list and an accumulator, producing the final value
of the accumulator.
...
Accepts a list `a` and a gate `b` which takes any noun and produces a
loobean.  
Produces the sublist of `a` whose elements produce true by slamming the
gate with them.
...
Accepts a list `a` and a gate `b` which takes any noun and produces a
loobean. Produces the sublist of `a` whose elments produce false when
slammed with `b`.
...
Accepts an atom `a` and list `b`, producing the last `a` elements from
...
Accepts a list `a` and a gate `b` with a sample which accepts two nouns
and produces a loobean. 'sort' then produces a list of the elements of
'a' sorted according to 'b'.
...
A range in a list - Produces the values in list 'c' starting at index
'a' and spanning 'b' elements more than that.

#### Summary

Build wet %gold gate with cell sample that accepts a cell of atoms, `a`
and `b`, and a list `c`. The last `a` elements in `c` are slammed to
slag. Scag is slammed with the atom `b`and the list just produced by
slag.
...
Accepts a list `a` and a gate `b`. Produces the list with the gate
applied to each element of the original list.
...
Concatenate two lists without losing their type information to
homogenization. Produces a tape when passed two tapes.
