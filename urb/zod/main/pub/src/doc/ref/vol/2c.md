...
Accepts a block size 'a', a cell of two atoms 'b' and 'c' and another
atom 'd'. Produces the tail of 'd' that is 'c' blocks long after
right-shifting 'd' 'b'-blocks.
...
++end takes a bloq `a`, and atoms `c` and `d`. It returns the tail of
`c`, whose length is determined by the number of bloqs `b`, of size `a`
...
Accepts a block size `a` and two atoms `b` and `c`. Produces `c`
left-shifted `b` times by the block size.
...
Produces a list of the bits of an atom, in little endian order,
...
Accepts a block size 'a' and two atoms, 'b' and 'c'. Right-shifts 'c' by
'b' blocks of size 'a'.
...
Produces the bit-wise logical NOT over 'b' blocks of the given
blocksize.
...
of size 5 off the product's end.
...
Hashes any noun with the 31-bit nonzero FNV-1a non-cryptographic hash
algorithm.
...
Produces a signed integer from an unsigned integer. Note that the result
must be manually cast to some @s odor to be inferred as an unsigned
integer in the type system.
...
Is a signed integer positive? Produce the sign of a signed integer - &
being posiitve, | negative.
...
~zod/try=\> -\<- ~2014.8.4..19.39.59..9288
...
Ruthlessly demands that a specific type be produced, crashing the
program is it is not.
